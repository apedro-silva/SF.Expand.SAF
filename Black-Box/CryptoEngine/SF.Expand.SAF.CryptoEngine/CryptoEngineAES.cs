	...
	public class CryptoEngineAES
	{
		...
		private void AddRoundKey(int round)
		{
			for (int i = 0; i < 4; i++)
			{
				for (int j = 0; j < 4; j++)
				{
					this.State[i, j] = (this.State[i, j] ^ this.w[round * 4 + j, i]);
				}
			}
		}
		...
		{
			return CryptoEngineAES.gfmultby02(CryptoEngineAES.gfmultby02(CryptoEngineAES.gfmultby02(b))) ^ b;
		}
		private static byte gfmultby0b(byte b)
		{
			return CryptoEngineAES.gfmultby02(CryptoEngineAES.gfmultby02(CryptoEngineAES.gfmultby02(b))) ^ CryptoEngineAES.gfmultby02(b) ^ b;
		}
		private static byte gfmultby0d(byte b)
		{
			return CryptoEngineAES.gfmultby02(CryptoEngineAES.gfmultby02(CryptoEngineAES.gfmultby02(b))) ^ CryptoEngineAES.gfmultby02(CryptoEngineAES.gfmultby02(b)) ^ b;
		}
		...
		private void KeyExpansion()
		{
			...
			for (int num = this.Nk; num < this.Nb * (this.Nr + 1); num++)
			{
				...
				if (num % this.Nk == 0)
				{
					...
					word[0] = (word[0] ^ this.Rcon[num / this.Nk, 0]);
					word[1] = (word[1] ^ this.Rcon[num / this.Nk, 1]);
					word[2] = (word[2] ^ this.Rcon[num / this.Nk, 2]);
					word[3] = (word[3] ^ this.Rcon[num / this.Nk, 3]);
				}
				else
				{
				...
				this.w[num, 0] = (this.w[num - this.Nk, 0] ^ word[0]);
				this.w[num, 1] = (this.w[num - this.Nk, 1] ^ word[1]);
				this.w[num, 2] = (this.w[num - this.Nk, 2] ^ word[2]);
				this.w[num, 3] = (this.w[num - this.Nk, 3] ^ word[3]);
			}
		}
